# Fit Crowder seeds with R-INLA and dump fixed-effect / b / τ marginals
# for cross-validation against Latte's INLA.
#
# Inputs:
#   seeds_data.csv    — plate, r, n, x1, x2
#   params.json       — { "pc_U", "pc_alpha", "strategy" }
#
# Outputs:
#   rinla_fixed_marginals.csv  — long form (name, x, density)
#   rinla_b_marginals.csv      — long form (i, x, density)
...
seeds <- read.csv(file.path(input_dir, "seeds_data.csv"))
params <- fromJSON(file.path(input_dir, "params.json"))
strategy <- params$strategy
n <- nrow(seeds)

cat(sprintf("R-INLA seeds: n = %d, strategy = %s\n", n, strategy))

# Crowder seeds: 2x2 design + plate-level iid RE.
formula <- r ~ x1 + x2 + x1:x2 + f(plate,
    model = "iid",
...
b_long <- do.call(rbind, lapply(seq_len(n), function(i) {
    m <- result$marginals.random$plate[[i]]
    data.frame(i = i, x = m[, "x"], density = m[, "y"])
}))
write.csv(b_long, file.path(output_dir, "rinla_b_marginals.csv"), row.names = FALSE)

# τ marginal (precision of the iid plate RE)
tau_marg <- result$marginals.hyperpar$`Precision for plate`
...
for (i in seq_len(n)) {
    s <- result$summary.random$plate[i, ]
    summary_rows[[length(summary_rows) + 1]] <- data.frame(
        name = sprintf("b[%d]", i), mean = s$mean, sd = s$sd,
        q025 = s$`0.025quant`, q5 = s$`0.5quant`, q975 = s$`0.975quant`
    )
}
