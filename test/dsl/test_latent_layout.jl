...
@testset "Latent layout from latte_from_dppl" begin
    ...
    @testset "Non-augmented fast-path: sym → base-latent range" begin
        ...
        lgm = latte_from_dppl(m(y, H); random = (:u,), augment = false)
...
@testset "linear_combinations(result; kwargs...) — named version" begin

    @testset "Equivalent to hand-built design matrix" begin
        @model function m(y, X, group, G)
            τ_u ~ Gamma(2.0, 1.0)
            β ~ MvNormal(zeros(size(X, 2)), 100.0 * I(size(X, 2)))
            u ~ MvNormal(zeros(G), (1 / τ_u) * I(G))
    ...
    @testset "Scalar coefficient is broadcast to a column of ones" begin
        ...
        lgm = latte_from_dppl(m(y, group, G); random = (:β, :u))
        result = inla(lgm, y; progress = false)
