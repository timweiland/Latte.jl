...
@testset "DPPL → latte_from_dppl → inla + tmb" begin
    ...
    # Use FiniteDiffStrategy + GaussianMarginal — the AutoDiffObservationModel
    # path currently nested AD triggers a PreparationMismatchError (a known
    # limitation of the DPPL adapter's obs-model wrapping — tracked for the
    # Phase 5 follow-up).
    inla_r = inla(
        model, y_obs;
        progress = false,
    ...
    @testset "Both results satisfy the protocol" begin
        for r in (inla_r, tmb_r)
            ...
            @test length(latent_marginals(r)) == p + G
    ...
    @testset "Latent posterior recovers truth within ~2 SE" begin
        true_x = vcat(β_true, u_true)
        for r in (inla_r, tmb_r)
            lm = latent_marginals(r)
            means = mean.(lm)
            stds = std.(lm)
            # All latent components should be within 3 SE of the truth (loose
