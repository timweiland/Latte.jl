...
@testset "Link Functions" begin
    ...
    @testset "LogitLink" begin
        ...
        @test apply_invlink.(Ref(link), logit.(p)) ≈ p rtol=1e-10
        
        # Test derivatives
        for pi in p
            η = logit(pi)
            expected_deriv = pi * (1 - pi)
            @test IntegratedNestedLaplace.derivative_invlink(link, η) ≈ expected_deriv rtol=1e-10
            
            expected_second_deriv = pi * (1 - pi) * (1 - 2*pi)
            @test IntegratedNestedLaplace.second_derivative_invlink(link, η) ≈ expected_second_deriv rtol=1e-10
        end
        
        # Compare with ForwardDiff
        for pi in p
            η = logit(pi)
            @test IntegratedNestedLaplace.derivative_invlink(link, η) ≈ ForwardDiff.derivative(x -> apply_invlink(link, x), η) rtol=1e-10
            @test IntegratedNestedLaplace.second_derivative_invlink(link, η) ≈ ForwardDiff.derivative(x -> IntegratedNestedLaplace.derivative_invlink(link, x), η) rtol=1e-10
        end
    end
