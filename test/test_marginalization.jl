...
@testset "Marginalization" begin
    ...
    @testset "Gaussian Likelihood - Laplace should match Gaussian exactly" begin
        ...
        for (i, idx) in enumerate(test_indices)
            ...
            @test mean(gauss_marginal) ≈ mean(laplace_marginal) atol=1e-6
            @test var(gauss_marginal) ≈ var(laplace_marginal) atol=1e-6
            
            # Check that PDFs match at several points
            test_points = [mean(gauss_marginal) + k*std(gauss_marginal) for k in -2:0.5:2]
            for x in test_points
                @test pdf(gauss_marginal, x) ≈ pdf(laplace_marginal, x) rtol=1e-4
            end
        end
    end
    ...
    @testset "Bernoulli Likelihood - Non-Gaussian Case" begin
        ...
        for (i, idx) in enumerate(test_indices)
            ...
            integral, _ = hcubature(x -> pdf(laplace_marginal, x[1]), [μ_l - 5*σ_l], [μ_l + 5*σ_l], rtol=1e-4)
            @test integral ≈ 1.0 atol=1e-2
        end
        ...
        @test mean_diff > 1e-4  # Should see some difference
    end
