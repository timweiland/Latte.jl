...
@testset "WeightedMixture Tests" begin
    ...
    @testset "Constructor Tests" begin
        ...
        @test mixture_normalized.weights ≈ [0.3, 0.7] atol = 1.0e-10
    ...
    @testset "PDF Tests" begin
        ...
        @test pdf(mixture, x) ≈ expected_pdf atol = 1.0e-10
        ...
        @test logpdf(mixture, x) ≈ log(pdf(mixture, x)) atol = 1.0e-10
    ...
    @testset "CDF Tests" begin
        ...
        @test cdf(mixture, x) ≈ expected_cdf atol = 1.0e-10

        # Test CDF properties
        @test cdf(mixture, -Inf) ≈ 0.0 atol = 1.0e-10
        @test cdf(mixture, Inf) ≈ 1.0 atol = 1.0e-10
    ...
    @testset "Moments Tests" begin
        ...
        @test mean(mixture) ≈ expected_mean atol = 1.0e-10

        # Variance: Var(X) = E[Var(X|θ)] + Var(E[X|θ])
        # E[Var(X|θ)] = Σ wᵢ σᵢ²
        # Var(E[X|θ]) = Σ wᵢ μᵢ² - (Σ wᵢ μᵢ)²
        expected_var = 0.4 * 4 + 0.6 * 1 + 0.4 * 1^2 + 0.6 * 4^2 - expected_mean^2
        @test var(mixture) ≈ expected_var atol = 1.0e-10

        # Standard deviation
        @test std(mixture) ≈ sqrt(expected_var) atol = 1.0e-10

        # Test single component reduces to component moments
        single_mixture = WeightedMixture([Normal(2, 3)], [1.0])
        @test mean(single_mixture) ≈ 2.0 atol = 1.0e-10
        @test var(single_mixture) ≈ 9.0 atol = 1.0e-10
    end
    ...
    @testset "Quantile Tests" begin
        ...
        @test cdf(mixture, median_val) ≈ 0.5 atol = 1.0e-6

        # Test quartiles
        q25 = quantile(mixture, 0.25)
        q75 = quantile(mixture, 0.75)
        @test cdf(mixture, q25) ≈ 0.25 atol = 1.0e-6
        @test cdf(mixture, q75) ≈ 0.75 atol = 1.0e-6
    ...
    @testset "Edge Cases and Error Handling" begin
        ...
        tiny_weights = [1.0e-10, 1.0 - 1.0e-10]
        ...
        @test mean(identical_mixture) ≈ 2.0 atol = 1.0e-10
        @test var(identical_mixture) ≈ 1.0 atol = 1.0e-10
        ...
        @test pdf(mixture_zero, 0.5) ≈ pdf(Normal(1, 1), 0.5) atol = 1.0e-10
    end

    @testset "Numerical Stability Tests" begin
        # Test with very different scales
        components = [Normal(0, 1.0e-6), Normal(1.0e6, 1.0e3)]
        weights = [0.5, 0.5]
        mixture = WeightedMixture(components, weights)

        # Should not throw or return NaN/Inf
        @test isfinite(pdf(mixture, 0.0))
        @test isfinite(pdf(mixture, 1.0e6))
        @test isfinite(logpdf(mixture, 0.0))
        @test isfinite(logpdf(mixture, 1.0e6))
        @test isfinite(cdf(mixture, 5.0e5))
        @test isfinite(mean(mixture))
        @test isfinite(var(mixture))
