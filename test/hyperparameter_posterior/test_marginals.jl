...
@testset "Marginal Posterior Computation" begin
    ...
    @testset "1D Marginals (Identity Case)" begin
        ...
        for test_val in test_values
            ...
            @test marginal_logpdf ≈ direct_logpdf atol=1e-6
        end
    end
    ...
    @testset "2D Marginal Consistency" begin
        ...
        function two_variance_latent(θ_named)
            ...
            Q = spdiagm(0 => fill(1/σ_latent^2 + 1e-6, n))  # Add small regularization
            return GMRF(zeros(n), Q, CholeskySolverBlueprint())
        end
    ...
    @testset "Marginal Integration Properties" begin
        ...
        function location_scale_latent(θ_named)
            ...
            Q = spdiagm(0 => fill(1/σ^2 + 1e-6, n))  # Add small regularization
    ...
    @testset "Integration Tolerance Effects" begin
        ...
        # Loose tolerance
        marginal_loose = hyperparameter_marginal_logpdf(posterior_approx, 1, test_val; rtol=1e-3, atol=1e-6)
        
        # Tight tolerance  
        marginal_tight = hyperparameter_marginal_logpdf(posterior_approx, 1, test_val; rtol=1e-6, atol=1e-10)
