...
@testset "Mode Finding" begin
    ...
    @testset "Optimality Conditions" begin
        ...
        @test abs(grad_at_mode[1]) < 1e-3  # Gradient should be near zero
    ...
    @testset "Robustness and Edge Cases" begin
        ...
        function exponential_latent(θ_named)
            λ = θ_named.λ
            n = 3
            Q = spdiagm(0 => fill(λ + 1e-6, n))  # Add small regularization
            return GMRF(zeros(n), Q, CholeskySolverBlueprint())
        end
        ...
        @test isfinite(θ_star[1])
        @test θ_star[1] ≈ 0.0 atol=1e-3  # Should be near boundary for extreme data
    end
