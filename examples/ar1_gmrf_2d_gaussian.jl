...
θ_prior = HyperparameterPrior((σ_gmrf=Gamma(2, 3), ρ=Uniform(0, 0.5)), fixed=(σ=1e-6,))
...
y_gt = rand(likelihood(obs_model, x_gt, (σ=1e-6,)))

println("Generated $(length(y_gt)) observations")

# Plot the time series
println("\nVisualizing AR-1 time series...")
p_ts = plot(y_gt, title="Observations", xlabel="Time", ylabel="y(t)",
...
exploration = explore_hyperparameter_posterior(inla_model, y_gt, θ_star, nothing, nothing;
                                              δ_π=2.5, interpolation_factor=3)
...
p1 = scatter(σ_vals, ρ_vals, zcolor=log_densities, 
...
scatter!(p1, integration_σ, integration_ρ,
...
# Mark integration points
scatter!(p2, integration_σ, integration_ρ,
...
p_diff = heatmap(σ_fine, ρ_fine, difference_grid',
                 xlabel="σ (marginal std dev)", ylabel="ρ (autocorrelation)",
                 title="Interpolation Error (Interp - True)")

# Add interpolation points
scatter!(p_diff, σ_vals, ρ_vals,
         markersize=3, color=:black, alpha=0.8, label="Interpolation Points")

# Mark the mode
scatter!(p_diff, [θ_star[1]], [θ_star[2]],
         marker=:star, markersize=8, color=:red, label="Mode")

# Create combined comparison plot
p_comparison = plot(p_interp, p_true, p_diff, layout=(1,3), size=(1800,500),
